...
def predict_vi_decomposed(regions,
                          V_params,
                          hyperparams,
                          M=100):
    """
    Same MC predictive marginal as ``predict_vi``, but also return the law-of-total-variance
    decomposition at each test anchor:

      var_pred = E_m[var_s] + E_m[(mu_s - mu_pred)^2]
               = within (local JumpGP variance) + between (projection / W-sample disagreement).

    Returns:
      mu_pred: [T]
      var_pred: [T]
      within_var: [T]  mean_m sig2_{m,t} from ``jumpgp_ld_wrapper``
      between_var: [T]  mean_m (mu_{m,t} - mu_pred_t)^2
    """
    T = len(regions)
    X_test = hyperparams['X_test']      # [T,D]
    mu_V = V_params['mu_V']             # [m2,Q,D]
    ...
    within_var = torch.nan_to_num(within_var, nan=1e-6)
    between_var = torch.nan_to_num(between_var, nan=0.0)
    var_pred = torch.nan_to_num(var_pred, nan=1e-6)

    return mu_pred, var_pred, within_var, between_var
...
def predict_vi(regions,
    ...
    Returns:
      ...
      var_pred: [T] tensor
    """
    mu_pred, var_pred, _, _ = predict_vi_decomposed(regions, V_params, hyperparams, M=M)
